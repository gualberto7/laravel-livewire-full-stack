<div>
    <form wire:submit="save" class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2">
            <flux:field>
                <flux:label for="max_installments">Máximo de cuotas</flux:label>
                <flux:input id="max_installments" wire:model="form.max_installments" type="number" min="1" placeholder="Opcional" />
                <flux:error name="form.max_installments" />
            </flux:field>

            <flux:field>
                <flux:label for="max_checkins">Máximo de entradas</flux:label>
        ...
        <div class="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2">
            @if($form->is_promo)
                ...
                <flux:field>
                    <flux:label for="max_clients">Máximo de clientes</flux:label>
                    <flux:input id="max_clients" wire:model="form.max_clients" type="number" min="0" placeholder="Opcional" />
                    <flux:error name="form.max_clients" />
                </flux:field>
            @endif
        </div>
